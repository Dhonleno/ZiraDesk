...
async function cleanupExpiredCsat(): Promise<void> {
  ...
  for (const tenant of tenants) {
    ...
    // Um tenant meio-provisionado (sem a tabela conversations) não pode abortar
    // a varredura — os tenants seguintes precisam ser processados mesmo assim.
    try {
      ...
      logger.error(
        ...
        '[CSAT Cleanup] Skipping tenant — failed to sweep expired CSAT',
      );
    }
  }
  ...
  if (skipped > 0) {
    logger.warn({ skipped, total: tenants.length }, '[CSAT Cleanup] Sweep completed with skipped tenants');
  }
}
