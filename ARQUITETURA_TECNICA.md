...
- **[BAIXO] `resetTenantTicketData` expôs vetor de deadlock Postgres `40P01` no CI (intermitente)** — o helper local de `tickets.integration.test.ts:47-60` executa `TRUNCATE TABLE ... RESTART IDENTITY CASCADE` sobre 7 tabelas de tickets, logo após `ensureTicketInfrastructureForSchema(schemaName)`. Na run `30838022935` (2026-08-03) o teste `POST /api/tickets com department_id e auto-assign ligado` falhou com `40P01 deadlock detected` — um processo pedindo `AccessExclusiveLock` bloqueado por outro segurando `AccessShareLock` na relação inversa (OIDs `29644` e `29599`). **Frequência medida: 1 falha em 2 tentativas do mesmo commit** — o re-run passou sem nenhuma alteração de código, o que classifica o caso como intermitente de paralelismo, não como reprodutível. **Sem vínculo causal com as mudanças da sessão** (`TENANT_SUSPENDED` em auth/tenant/web): o helper não foi tocado, e nenhum teste da suíte assere sobre as mensagens alteradas. **Mecanismo exato não estabelecido** — o registro abaixo separa o observado do inferido, porque duas hipóteses concorrem e apontam para correções diferentes. Como cada worker opera em seu próprio `schemaName`, um `TRUNCATE` de schema A não deveria colidir com o de schema B; logo o vetor mais provável **não é a ordem das 7 tabelas listadas** (que já é fixa e tem uma só definição, local ao arquivo — não há múltiplos call sites a alinhar), e sim (i) o `CASCADE`, que estende o truncate a tabelas com FK **não listadas**, em ordem escolhida pelo Postgres e não pelo autor, podendo alcançar relação compartilhada; ou (ii) o `ALTER TABLE`/`CREATE TABLE` de `ensureTicketInfrastructureForSchema` imediatamente anterior, que toma `ACCESS EXCLUSIVE` — o vetor descrito no item de DDL lazy acima. Não bloqueia deploy: o gate do CI barra o Deploy Contabo enquanto vermelho, e o re-run destravou normalmente. O registro existe porque o vetor `40P01` é novo — não estava catalogado junto de `0A000`/`42P01` — e porque a mensagem de erro com os OIDs é a pista mais barata para quem for investigar. Candidatos a avaliar em tarefa própria, na ordem em que atacam as hipóteses: **(a)** confirmar por `pg_locks`/log do servidor quais relações são as 2 do deadlock, o que decide entre (i) e (ii) e evita corrigir às cegas; **(b)** remover o `CASCADE`, listando explicitamente toda tabela dependente, para que o conjunto de locks seja o declarado; **(c)** trocar `TRUNCATE` por `DELETE` neste helper — perde o reset de sequence do `RESTART IDENTITY`, ganha lock bem menos agressivo; **(d)** isolar `tickets.integration.test.ts` em execução não-paralela, que mascara sem corrigir. Irmão da família já registrada de dívida sobre DDL lazy e paralelismo (`0A000` reproduzido 2/19 runs, `42P01` do Grupo B).
