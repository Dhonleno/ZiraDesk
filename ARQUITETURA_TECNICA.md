...
- O Origin Certificate atual cobre `ziradesk.com` e `*.ziradesk.com`, mas nao
  cobre `*.*.ziradesk.com`; isto e limitacao estrutural do Cloudflare Origin
  Certificate, nao pendencia de regeneracao do certificado.
...
### Sprint 1 — Super Admin (2-3 dias) ⚠️ ~90% (gap em métricas de negócio)
- [x] CRUD de planos
- [x] CRUD de tenants
- [x] Ativar/suspender tenant
- [ ] Dashboard com métricas globais — **parcial**: falta churn e uso agregado por plano; MRR existe mas está em `Tenants.tsx` (via `GET /tenants/stats`), não no Dashboard
- [x] Tela de Super Admin (frontend)
...
> **Notas de boot em producao (migração 2026-08-02):**
> - `META_APP_SECRET` e validado como obrigatorio no boot mesmo sem canal
>   WhatsApp ativo; usar placeholder nao-vazio ate configurar a credencial real.
> - `RESEND_FROM_EMAIL` rejeita string vazia por validacao de formato; usar um
>   e-mail de formato valido ate `RESEND_API_KEY` ser preenchida.
...
- **[INFRA] Migração concluída para novo Contabo VPS em 2026-08-02, com pendências de automação** — o VPS Contabo anterior (`85.239.245.8`) foi perdido por suspensao ligada a metodo de pagamento. Depois de avaliar Hetzner (estoque indisponivel na linha Cost-Optimized), Vultr, DigitalOcean e Magalu Cloud (custo/regiao inadequados ao caso), a infraestrutura foi recriada na propria Contabo. Novo servidor: Contabo Cloud VPS 6, linha **Core** (nao Performance), 6 vCPU, 12GB RAM, 200GB SSD, regiao US-East, IP `66.94.105.48`, hostname `vmi3482143`. A escolha por Core foi consciente: o workload e mais I/O/RAM-bound que CPU-bound; Core custa menos da metade do Performance para vCPU/RAM equivalentes, e a limitacao de I/O por overselling da Contabo afeta as duas linhas de forma parecida, entao Performance nao compraria a consistencia ausente. O `vps-bootstrap.sh` existente foi reaproveitado com sucesso e configurou `deploy` com sudo NOPASSWD/grupo docker, SSH hardening, UFW, fail2ban, swap 4GB, timezone `America/Sao_Paulo`, Docker CE e Compose plugin. Achado operacional: na imagem Ubuntu 24.04 da Contabo, drop-ins em `/etc/ssh/sshd_config.d/` sao lidos antes do `/etc/ssh/sshd_config`, e no OpenSSH a primeira ocorrencia da diretiva vence; `50-cloud-init.conf` trazia `PasswordAuthentication yes`, entao editar so o arquivo principal ou rodar o bootstrap nao bastava. Foi corrigido manualmente com `sed -i 's/PasswordAuthentication yes/PasswordAuthentication no/' /etc/ssh/sshd_config.d/50-cloud-init.conf`; `PermitRootLogin` nao tinha conflito e ficou coberto pelo bootstrap. Para futuras provisoes, rodar sempre `ls -la /etc/ssh/sshd_config.d/` e `sshd -T | grep -Ei 'permitrootlogin|passwordauth'` antes/depois do hardening. O gap "VPS crua -> repo clonado" foi documentado em `docs/technical/DEPLOY_VPS_DOCKER_COMPOSE.md` no novo "Passo 0", incluindo copia do bootstrap, cuidado com CRLF, checagens de apt/swap, sessao root salva-vidas, validacao de `deploy`, deploy key dedicada read-only no GitHub, clone e diretorios persistentes. Dois requisitos de boot tambem foram registrados: `META_APP_SECRET` e obrigatorio mesmo sem WhatsApp ativo, e `RESEND_FROM_EMAIL` precisa ter formato valido mesmo sem `RESEND_API_KEY`. Certificado TLS: Origin Certificates da Cloudflare nao suportam wildcard de dois niveis (`*.*.ziradesk.com`) nem wildcard interno; o certificado atual cobre `ziradesk.com` e `*.ziradesk.com` ate 2041, que e o maximo possivel via Origin CA. Assim, `suporte.{tenant}.ziradesk.com` continua sem TLS de origem valido por limitacao de design ja existente, nao por regressao desta migracao. Alternativas futuras: enumerar hostnames por tenant, investigar Cloudflare Advanced Certificate Manager, ou redesenhar a URL do portal para evitar dois niveis de subdominio. Pendencias abertas: o IP ja foi corrigido para `66.94.105.48` em `.github/workflows/deploy-contabo.yml` e `.github/workflows/backup-manual.yml`, mas os secrets GitHub `CONTABO_SSH_PRIVATE_KEY` e `VPS_SSH_KEY` ainda referenciam a chave do servidor antigo e precisam ser regenerados — ate essa troca os dois workflows falham na autenticacao SSH mesmo com o host correto; cron de backup (`rclone` + `pg_dump` para R2) nao foi reconfigurado; teste funcional completo de login via seed super admin ainda nao foi executado.
  **Atualização (verificada em Deploy Contabo #213, run 30822031385, 2026-08-03):** `CONTABO_SSH_PRIVATE_KEY` já está funcional — os passos "Configure SSH for Contabo VPS" e "Deploy to Contabo VPS" do `deploy-contabo.yml` executaram e completaram com sucesso contra o host correto, incluindo `git pull --ff-only origin main`, rebuild dos containers, `api-migrate` e health check da API. A afirmação acima não se aplica mais a esse secret. `VPS_SSH_KEY` permanece não verificado: `backup-manual.yml` é `workflow_dispatch` apenas e não rodou nesta ocasião, então a pendência de regeneração continua valendo para ele.
  **[MÉDIO] DÍVIDA RESIDUAL — verificação de backup não cobre endpoint mal configurado.** O commit `5ac7886` adicionou verificação pós-upload em `ops/backup.sh`: `rclone lsf` após cada `copy` (os dois diários e os dois mensais), abortando com log distinto para falha de listagem e para objeto ausente. Fecha o caminho "`copy` no-op / objeto ausente no destino", mas **não** detecta endpoint do rclone mal configurado — `lsf` e `copy` usam o mesmo `--config`, então um path fantasma engana os dois: o `lsf` lista o mesmo destino errado e confirma o objeto. Metade da causa do incidente original (endpoint malformado gravando em path fantasma) portanto continua passando pela verificação; o endpoint segue validado manualmente na config do rclone, não pelo script. Critério de aceite pendente: (i) ao configurar o cron/systemd do servidor, não exportar `R2_REMOTE`, deixando prevalecer o default corrigido do script (`r2:ziradesk-backups-prod`) — exportar a variável por cima deixaria o fix inerte; (ii) exercitar a verificação contra o R2 real, nos dois caminhos de erro, antes do primeiro backup de produção; (iii) rodar `shellcheck` no servidor — não estava disponível na máquina de dev nas três rodadas de edição, então o script só passou por `bash -n`. Irmão da pendência "cron de backup (`rclone` + `pg_dump` para R2) nao foi reconfigurado" registrada acima neste mesmo item. **Atualização (2026-08-03, mesma sessão): os 3 critérios de aceite acima foram cumpridos.** (i) Cron configurado às 03h00 **sem exportar `R2_REMOTE`**, confirmado via `crontab -l` — o default corrigido do script (`r2:ziradesk-backups-prod`) prevalece. (ii) `verify_uploaded` exercitado contra o R2 real nos **dois ramos**: objeto ausente aborta com `exit 1` pelo `grep`, e o caminho de sucesso foi confirmado por `rclone lsf` em execução real. (iii) `shellcheck` rodou **limpo (exit 0)** na versão sincronizada em `/home/deploy/scripts/backup.sh`, fechando o gap de a máquina de dev não ter a ferramenta. A re-cópia de `backup.sh` e `restore.sh` para `/home/deploy/scripts/` também foi confirmada idêntica após a correção de bucket do `restore.sh` (`4a98218`), então o caminho que executa e o repositório estão sincronizados nesta data — sujeitos a divergir de novo no próximo deploy que alterar `ops/*.sh`, pelo motivo do item de divergência de caminho acima. **A limitação estrutural permanece válida e continua sem teste**: endpoint mal configurado engana `lsf` e `copy` igualmente, e reproduzir isso com segurança exigiria apontar a config para um destino falso — não foi feito.
  **[MÉDIO] Causa-raiz adicional do mesmo item — `ops/*.sh` não é o que roda em produção.** Descoberto em 2026-08-03, depois do deploy de `5ac7886`: `/home/deploy/scripts/backup.sh` e `/home/deploy/scripts/restore.sh` são **cópias manuais** de `ops/backup.sh`/`ops/restore.sh` — arquivos regulares, **não symlinks** (confirmado por `ls -la /home/deploy/scripts/`). O `git pull --ff-only origin main` do `deploy-contabo.yml` atualiza apenas `~/ziradesk/app/ops/`, e nenhum passo do deploy propaga para `/home/deploy/scripts/`, que é o caminho de fato invocado pelos três caminhos de execução (cron das 03h00, `backup-manual.yml` e SSH manual). Consequência medida: o Deploy Contabo #213 completou com `success` e mesmo assim o backup de produção seguiu rodando a versão antiga — sem `verify_uploaded` e apontando para o bucket errado — até a re-cópia manual ser feita nesta mesma data. **Deploy verde não implica script de backup atualizado.** O procedimento manual (`cp` das duas cópias + `chmod +x`) foi documentado em `docs/technical/DEPLOY_VPS_DOCKER_COMPOSE.md`, mas continua sendo passo humano e falível — foi exatamente o passo esquecido desta vez. **Follow-up estrutural pendente:** adicionar step de sincronização ao `deploy-contabo.yml` (o `cp` logo após o `git pull`, versionando de fato o que executa) ou trocar as cópias por symlinks para `~/ziradesk/app/ops/`. Enquanto não for feito, qualquer correção futura em `ops/*.sh` nasce inerte em produção até alguém lembrar da cópia — e isso enfraquece na prática os critérios de aceite (ii) e (iii) do parágrafo acima, que passam a valer para a cópia em `/home/deploy/scripts/`, não para o arquivo do repositório.
- **[MÉDIO] GitHub Actions não cria run para push em `main` — sem diagnóstico** — no deploy da Fase 1 (2026-08-06), o Actions **não criou nenhum run** para o commit `49a4fdf`, apesar de todas as pré-condições estarem satisfeitas: `main` apontando para o commit (confirmado via `GET /repos/.../branches/main`), ambos os workflows em `state: active` (`GET /repos/.../actions/workflows`), e o push registrado com sucesso (`a771068..49a4fdf  main -> main`). A consulta `GET /repos/.../actions/runs?head_sha=49a4fdf...` retornou `total_count: 0`. **Não é fila lenta:** o commit anterior (`a771068`) levou ~3 minutos do push ao deploy concluído (CI run #271 criado `13:30:28Z`, Deploy #232 às `13:33:21Z`); aqui passaram ~25 minutos sem sequer um run enfileirado. O `workflow_dispatch` manual **também falhou**: run `31127984226` disparou (`event: workflow_dispatch`) mas o job terminou `cancelled` em exatos 15 minutos (`20:58:37Z` → `21:13:40Z`), e o `FETCH_HEAD` da VPS permaneceu em `Aug 6 10:33` — prova de que o script remoto **nunca chegou ao `git fetch`**, ou seja, a falha foi no estabelecimento da conexão, antes de qualquer comando no servidor. **Consequência real, não apenas inconveniência:** foi esta anomalia que forçou o deploy **manual via SSH**, e foi a transcrição truncada desse script manual que causou o incidente `502` registrado no item abaixo. A dívida do CI já teve, portanto, consequência de produção — ao empurrar o deploy para um caminho mais frágil. **Hipóteses não descartadas** (não foi possível ler os logs do run: exigem `gh`/token autenticado, indisponíveis no ambiente): degradação transitória do Actions; problema de conectividade runner→VPS específico daquela janela; mudança de configuração de workflow ou branch protection; limite de minutos/billing. Descartadas por medição na VPS: `git fetch` funciona (trouxe `49a4fdf` em segundos), disco em 26%, load 0.67, nenhuma falha de SSH no `auth.log` da janela, e nenhum IP de runner GitHub entre os 10 banidos pelo fail2ban. **Ação pendente:** investigar com acesso autenticado ao GitHub Actions — logs do run cancelado, histórico de entrega de webhooks de push do repositório, e status de billing/minutos. **Contorno até lá:** deploy manual via SSH replicando o script **completo** do `deploy-contabo.yml` — incluindo as linhas 96–99 (`nginx -t`, `nginx -s reload`, smoke test de `/health`, `ps`) e **todos** os `</dev/null` — e nunca um recorte, para não repetir o `502`.
  **Atualização (2026-08-07, push seguinte):** o push de `a767b80` (commit de documentação) **disparou normalmente** — run criado imediatamente (`total_count: 1`), CI e Deploy Contabo concluídos com `success` em ~4 minutos, consistente com o histórico (`a771068`, ~3 min). Contraste direto com `49a4fdf`: 25 minutos de silêncio, zero runs. **Isso estreita o espaço de hipóteses sem fechar o diagnóstico.** *Enfraquece* branch protection e billing/cota como causa: fossem configuração persistente ou cota estourada, `a767b80` teria falhado igual. *Reforça* degradação transitória do Actions na janela do `49a4fdf` — o gatilho voltou sozinho, sem nenhuma intervenção de configuração no repositório. **Veredito permanece "não diagnosticado"**, porque os logs do run continuam sem ter sido lidos e a causa raiz não foi confirmada. Encaminhamento: se não recorrer, é candidato a fechar como *transitório não reproduzível*; se recorrer, há agora **dois SHAs para comparar** (`49a4fdf` falhou, `a767b80` passou) e a diferença entre eles passa a ser a pista mais direta. Verificação lateral do mesmo push: os containers `api`/`web`/`marketing` **foram de fato recriados** (`StartedAt` 13:27) e o nginx recarregou sem reiniciar (`StartedAt` de 02/ago preservado, master PID `1`), com as 13 locations dos 3 upstreams respondendo sem `502` — o que confirma empiricamente, num deploy real, que o `deploy-contabo.yml` cobre o gatilho *deploy* do item de cache de IP de upstream abaixo.
- ~~**[ALTO] `502 Bad Gateway` em todas as locations servidas por `ziradesk_api` durante o deploy da Fase 1**~~ — **✅ Resolvido (2026-08-06/07)**: por ~4 minutos, `api.ziradesk.com` e **todas as 9 locations do upstream `ziradesk_api`** ficaram em `502` — ou seja, todo `/api/` e todo `/socket.io/` de **todos** os domínios (`app.`, `api.`, `ziradesk.com`, o regex de tenant `~^(?<tenant>[a-z0-9-]+)\.ziradesk\.com$` e o de portal `~^suporte\.(?<tenant>...)`), o que inclui o painel de todos os tenants e o webhook da Meta. O escopo real ficou mascarado porque `/` é servido por `ziradesk_web`/`ziradesk_marketing` e continuou retornando `200`: o front carregava normalmente enquanto cada chamada de API falhava. Lição de diagnóstico: **testar `/` não prova que o upstream de API está vivo** — verificar `/health`, `/api/*` e `/socket.io/` explicitamente. **Causa real, registrada com precisão porque a versão errada induziria a "corrigir" o que já está certo: NÃO foi falha do `deploy-contabo.yml`.** O workflow já trata o caso desde `1d1cd14` — após o `up -d` final executa `nginx -t` (linha 96) + `nginx -s reload` (linha 97) e ainda um smoke test `wget http://127.0.0.1:3333/health` (linha 98) que faria o job falhar caso o `502` ocorresse. O incidente aconteceu porque o deploy foi executado **manualmente via SSH** (o workflow do GitHub Actions não disparava — ver item de dívida do CI abaixo) e o script manual foi transcrito com `sed -n '40,95p'`, **truncando em 95 e omitindo as linhas 96–99**. Sem o reload, o nginx manteve em cache o IP antigo do container `api` (resolvido na carga da config, em 2026-08-02), enquanto o `api` recriado recebeu `172.18.0.7`. Corrigido com `docker exec ziradesk-nginx nginx -s reload`; as 13 locations dos 3 upstreams voltaram a responder (`/health` `200`, `/api/webhooks/whatsapp` `403` sem token, `/socket.io/` `400`, `/` `200`). **Gotcha relacionado, verificado na prática:** `docker compose exec -T` **sem `</dev/null`** consome o stdin subsequente do script chamador — o comando seguinte simplesmente não roda, silenciosamente. Foi reproduzido durante o teste manual: um `nginx -t` sem redirecionamento engoliu a linha do `nginx -s reload` logo abaixo. Todos os `exec -T` do workflow já usam `</dev/null` por esse motivo; **replicar o redirecionamento em qualquer execução manual do script de deploy**. **Status:** resolvido, sem alteração de código — o `deploy-contabo.yml` está correto e não deve ser modificado por causa deste incidente.
- **[MÉDIO] Cache de IP de upstream do nginx não é coberto em restart fora de banda** — dívida residual do incidente `502` acima, e o único pedaço dele que permanece aberto. O `nginx -s reload` do `deploy-contabo.yml` cobre o gatilho **deploy**; **não** cobre recriação de container fora desse caminho. Os serviços têm `restart: unless-stopped`, então se o `api` morrer por OOM, crash ou reboot do host e voltar com IP novo sem passar por deploy, o nginx segue apontando para o endereço morto **até o próximo deploy** — e de forma silenciosa, porque o front continua servindo `/` e mascarando a falha (exatamente o mecanismo que quase escondeu o incidente original). **Correção estrutural:** `resolver 127.0.0.11` (DNS embutido do Docker, confirmado no `/etc/resolv.conf` do container) + `proxy_pass` com variável por location, **abandonando os três blocos `upstream` nomeados** — no nginx open source (hoje `nginx/1.27.5-alpine`) a diretiva `resolver` **não funciona dentro de bloco `upstream`**, que é recurso do NGINX Plus; por isso a conversão precisa ser por-location. Escopo: `ziradesk_api` (9 locations), `ziradesk_web` (3), `ziradesk_marketing` (1) — **cobrir os três ou não fazer**, senão troca-se uma bomba-relógio por duas, já que `web` e `marketing` só não quebraram no incidente porque recuperaram os mesmos IPs (`172.18.0.4`/`.5`) por sorte na ordem de alocação do Docker, não por imunidade. Facilitador levantado na auditoria: **nenhum `proxy_pass` atual tem path ou `rewrite`** (todos são `http://ziradesk_xxx` puro), então a conversão é mecânica (`http://$var$request_uri`) sem alterar semântica de roteamento. A config vive em volume (`./deploy/nginx/conf.d` → `/etc/nginx/conf.d:ro`), então a mudança é editar + reload, sem rebuild. **Tratar como tarefa própria, com janela e verificação das 13 locations** — mexer em 13 pontos de roteamento de produção às pressas é precisamente o tipo de desvio que esta seção existe para registrar.
  **Atualização (2026-08-07) — conversão aplicada em `c70c7b3` e corrigida em `3a4c6dd`, com um incidente `502` no meio; a dívida PERMANECE ABERTA até a verificação em produção.** A conversão foi feita: os 3 blocos `upstream` saíram, os 13 `proxy_pass` viraram `http://$backend_*$request_uri` e cada um dos 5 server blocks `:443` declara `set $backend_*`. Provada em ambiente isolado (`nginx:1.27-alpine`, A/B contra a config antiga): com o IP do container `api` indo de `172.26.0.2` para `172.26.0.7` **sem reload**, a config nova respondeu `200` nas 9 locations de API e a antiga `502` nas mesmas 9, com `/` em `200` nas duas — reproduzindo inclusive o mascaramento pelo front. **Mas `c70c7b3` derrubou produção por ~7 minutos, por um mecanismo que esta seção não previa.** O `resolver` foi colocado em `nginx.conf`, e **`nginx -s reload` não o releu**. Os dois arquivos são bind mount, mas de tipos diferentes: `conf.d` é mount de **diretório** e reflete o `git pull`; `nginx.conf` é mount de **arquivo único**, preso ao inode original — e o `git pull` **substitui** o arquivo (escreve e renomeia) em vez de editá-lo, então o container seguiu enxergando o `nginx.conf` antigo. O nginx passou a usar o `ziradesk.conf` novo (`proxy_pass` por variável) contra um `nginx.conf` velho sem `resolver`, e o `error.log` registrou `no resolver defined to resolve web/api/marketing` com `/etc/resolv.conf` do container em `127.0.0.11` — ou seja, **o DNS estava certo; a diretiva é que não tinha sido lida**. Escopo maior que o do incidente original: os **três** upstreams caíram juntos, `/` inclusive, então nem houve mascaramento. Rollback com `git checkout a767b80 -- deploy/nginx/` + reload. **Correção (`3a4c6dd`):** `resolver` movido para dentro dos 5 server blocks de `conf.d/ziradesk.conf`, de modo que a mudança inteira viva no mount de diretório, e `deploy-contabo.yml` passa a recriar o nginx com `--force-recreate --no-deps`, o que refaz o mount de arquivo. **Lição que generaliza:** mount de arquivo único fica obsoleto quando o arquivo é substituído no host — `nginx -s reload` sozinho nunca basta para `nginx.conf` depois de um `git pull`. **Segundo achado, independente:** o smoke test do `deploy-contabo.yml` rodava `wget` **dentro** do container `api` contra `127.0.0.1:3333`, sem atravessar o nginx — por isso o deploy podia terminar **verde com a borda inteira em `502`**. `3a4c6dd` adiciona um step que sai pela borda pública, com um probe por upstream. **Três pendências para fechar esta dívida:** (i) confirmar que `3a4c6dd` chegou à VPS — o rollback deixou modificações locais em `deploy/nginx/`, e o `git pull --ff-only` do deploy aborta sob `set -euo pipefail` enquanto elas não forem limpas; (ii) confirmar `grep -c "resolver 127.0.0.11" /etc/nginx/conf.d/ziradesk.conf` = `5` **dentro do container**, não só no host; (iii) executar a prova de fechamento em produção — canário no `marketing`, depois `api`, esperando `healthy` e confirmando absorção do IP novo sem reload, conforme o runbook em `docs/technical/DEPLOY_VPS_DOCKER_COMPOSE.md`. Enquanto (iii) não passar, nada aqui está verificado em produção.
- ~~**[ALTO] Incidente WhatsApp Meta `#194` ao salvar canal em produção**~~ — **✅ Resolvido (2026-08-04)**: a causa raiz era `WHATSAPP_VERIFY_TOKEN` vazio em `apps/api/.env.production`. O schema de env exigia presença (`z.string()`), mas não não-vazio, então o boot aceitava a variável e a falha só aparecia no fluxo de configuração do webhook: a chamada à Meta Graph API recebia `callback_url` junto de `verify_token` vazio e retornava `#194` ("Requires all or none of the params: callback_url, verify_token"). O token foi preenchido em produção sem expor o valor; a verificação pública de `GET https://api.ziradesk.com/api/webhooks/whatsapp` com `hub.mode=subscribe` e o token carregado do próprio `.env.production` passou a retornar `200`. **Gotcha operacional confirmado:** `docker compose restart api` reinicia o processo, mas não relê `--env-file`; após o primeiro restart o webhook continuou `403`. A aplicação do env novo exigiu `docker compose --env-file .env.production -f docker-compose.production.yml up -d --force-recreate --no-deps api`, que recriou o container, voltou `healthy` e fez a verificação retornar `200`. **Achado lateral fechado:** `.env.production` na raiz e em `apps/api/` estavam não rastreados, mas também não cobertos por `.gitignore`; `git log --all --full-history -- "**/.env.production" "**/.env.development" "apps/*/.env.production" "apps/*/.env.development"` retornou vazio, então não houve vazamento histórico confirmado, mas um `git add -A` futuro poderia stagear secrets reais. Corrigido no commit `1380342` com `.env.production*`/`.env.development*` e exceções para `.example`. O arquivo órfão `apps/api/.env.production.save`, datado de 2026-08-02, sem referência em scripts/docs e diferente do `.env.production` atual, foi removido do servidor; o backup válido criado no incidente (`apps/api/.env.production.bak-20260804-203426`) foi mantido. Operações em produção de preenchimento do token e remoção do `.save` não têm hash Git porque alteraram apenas arquivos não versionados no servidor; o registro versionado fica neste item e no changelog. **Status:** resolvido, sem ação pendente.
- ~~**[ALTO] `Authorization Error` (OAuthException `100`) ao configurar canal WhatsApp em produção**~~ — **✅ Resolvido (2026-08-04)**: segunda causa raiz da mesma jornada de configuração de canal, independente do `WHATSAPP_VERIFY_TOKEN` vazio registrado no item acima — os dois incidentes se sobrepuseram e foram tratados como um só até a instrumentação separá-los. A causa desta metade era o Access Token do tenant **sem o escopo `whatsapp_business_management`** no Meta Business Manager. O que tornava o diagnóstico enganoso é que o token passava nos dois primeiros passos do fluxo: `debug_token` valida se o token existe e é válido, não os escopos que carrega; `app_subscriptions` autentica com credencial de **app** (`{appId}|{appSecret}`), não com o token do tenant, então também passava. A falha só aparecia no terceiro passo, `waba_fields` (`GET /{wabaId}?fields=id`), primeira chamada que de fato usa o token do tenant contra um recurso do WABA. A Meta respondia `OAuthException` código `100`, **sem `error_subcode`**, com a mensagem genérica `Authorization Error` — nenhum dos campos apontava para escopo. Três tentativas registradas em produção (2026-08-04, 21:28:00, 21:31:52 e 21:32:01 −03), todas parando no mesmo passo, com `fbtrace_id` distintos (`Azp5_RCXqEjhBm07fH_sTL0`, `AtxUa8DJ9Cy-JahkNSErzvO`, `AqZ6vBBunwl413qYC2gsWE4`) — confirmando requisições reais e distintas à Meta, não retry interno. **Nota operacional para diagnóstico futuro:** `Authorization Error` / `OAuthException` código `100` sem `error_subcode` ao configurar canal WhatsApp geralmente indica **escopo de permissão ausente no token** (`whatsapp_business_management`) ou **WABA não atribuído ao usuário de sistema** que gerou o token — não é necessariamente credencial errada ou digitada incorretamente. Verificar em Meta Business Manager → Usuários do sistema → Ativos atribuídos **antes** de suspeitar de bug no código. **Instrumentação permanente adicionada:** o commit `6ac4409` introduziu logging estruturado por passo em `apps/api/src/modules/admin/channels/channels.service.ts`, cobrindo os sete pontos do fluxo (`debug_token`, `app_subscriptions`, `waba_fields`, `phone_numbers`, `subscribed_apps`, `phone_webhook_config`, `phone_webhook_verify`) e emitindo `type`/`code`/`error_subcode`/`fbtrace_id` da Meta junto do `step`. Foi essa instrumentação que isolou o passo exato e separou as duas causas; sem ela, a mensagem genérica da Meta não distinguia qual chamada falhava. O log **não expõe credenciais** — o objeto serializado carrega apenas os cinco campos do erro, o token nunca entra na linha, e o `redact` do pino cobre `accessToken`/`token` como segunda barreira. Fica no código permanentemente, útil para diagnosticar qualquer tenant futuro com problema semelhante no onboarding. **Status:** resolvido — canal WhatsApp configurado com sucesso após ajuste de escopo no Meta Business Manager. A correção final é **ação de console externo, sem commit associado**; o registro versionado desta metade fica neste item, no changelog e no commit da instrumentação.
...
- ~~**[MÉDIO] Race conditions transitórias na suite de testes atribuídas a Socket.io ou pool Postgres**~~ — **hipótese refutada; classe `0A000` em fechamento por vetores**: a hipótese de Socket.io/pool genérico foi refutada por medição. A causa confirmada é colisão de plano cacheado por drift de shape entre schemas: uma conexão Prisma com plano cacheado para um schema pode reutilizar `SELECT *`/`RETURNING *` contra outro schema com result type físico diferente e disparar `0A000 cached plan must not change result type`. A correção das 16 colunas do Grupo A foi válida, mas parcial: adicionou ao provisionamento as colunas lazy de `contacts`/`users`, `conversations.department_id`, `tickets.sla_warning_sent_at`, `ticket_attachments.contact_id` e `bot_options.department_id`, reduzindo a frequência e fechando a divergência mais visível do export LGPD de contato. Porém investigação posterior reproduziu `0A000` 2 vezes em 19 execuções paralelas, e prova controlada com `SELECT * FROM conversations/tickets LIMIT 0` entre `tenant_demo` e `test_1785238505693` confirmou drift físico residual em schemas limpos. Vetor `conversations` por `RETURNING *` fechado na Frente B por qualificação (`conversationsRef`), inclusive para o caso especial de enum OID per-schema; higiene física de colunas lazy como `outbound_expires_at`, `outbound_origin_agent_id`, `outbound_returned_at`, `routing_started_at` e `routing_used_skill_id` segue pendente. Instâncias ainda abertas incluem `tickets` com ordem histórica divergente de colunas lazy como `department_id`, `level`, `sla_warning_sent_at` e campos de SLA/CSAT. Decisão pendente de dimensionamento: provisionar e ordenar fisicamente todas as colunas lazy em todas as tabelas afetadas, ou eliminar/qualificar `SELECT *`/`RETURNING *` em tabelas tenant-scoped. Schemas fantasmas de teste também contribuem para uma fatia das ocorrências e devem ser eliminados pela higiene de ambiente mapeada junto ao `42P01`. PgBouncer não foi usado porque silenciaria o sintoma sem corrigir a divergência de shape.
...
- **[MÉDIO] Deadlock `40P01` no CI: `TRUNCATE` do teste versus query do servidor de teste, no schema de tenant compartilhado** — **diagnóstico completo em 2026-08-03; NÃO corrigido nesta sessão, correção fica para tarefa dedicada.** O teste `POST /api/tickets com department_id e auto-assign ligado` (`tickets.integration.test.ts`) falha com `40P01 deadlock detected`, sempre nas mesmas duas relações (OIDs `29644` e `29599`, constantes entre execuções). **Frequência real: 2 falhas em 4 execuções** desta sessão — `b8b679e` tentativa 1 e `a07b9e0` tentativa 1; os re-runs passaram sem alteração de código. *(Registro anterior dizia "1 em 2" e severidade `[BAIXO]`, classificação feita sobre amostra pequena antes da investigação; corrigido aqui.)* **Sem vínculo causal com as mudanças da sessão** — `a07b9e0` é um commit de markdown puro e falhou igual; o helper nunca foi tocado.
  **Mecanismo confirmado, e não é nenhuma das três hipóteses iniciais.** Não é `CASCADE`, não é o DDL lazy de `ensureTicketInfrastructureForSchema`, e não é paralelismo entre arquivos de teste — `vitest.config.ts` tem `fileParallelism: false`, então os arquivos são serializados. O segundo processo é o **servidor Fastify real** que o `globalSetup` sobe em processo próprio (`test/setup.ts`, `app.listen({ port: 0 })` via `bootstrapIntegrationSuite`), e que atende as requisições HTTP dos testes enquanto o processo de teste manipula o banco. Ambos operam sobre o **mesmo schema de tenant compartilhado** (`test_${timestamp}`, criado uma única vez no `globalSetup` e consumido por 5 arquivos: `tickets`, `crm`, `portal`, `notifications`, `ticket-sla-warning`). A leitura dos locks fecha o quadro: o processo do teste detém `AccessExclusive` em `29599` e pede em `29644` — é o `TRUNCATE` multi-tabela adquirindo lock tabela a tabela na ordem em que estão listadas; o processo do servidor detém `AccessShare` em `29644` e pede em `29599` — é uma query da requisição em voo tocando as mesmas duas tabelas na **ordem inversa**. Inversão clássica de ordem de lock entre dois processos, no mesmo schema. Isso também explica a intermitência de ~50%: depende de haver requisição em voo no instante exato do `TRUNCATE`. **`fileParallelism: false` dá falsa sensação de serialização** — serializa os arquivos de teste, mas não o servidor, que roda concorrente a todos eles.
  **Distinto da família `0A000`/`42P01` já registrada acima**, apesar de conviverem: aquela é cache de plano e drift de shape **entre schemas diferentes** (`0A000` reproduzido 2/19 runs); esta é contenção de lock **entre processos no mesmo schema**, e é mais frequente. Confirmado por sonda local que as relações não vêm das migrations: um Postgres efêmero com as migrations aplicadas tem todo o `public` na faixa `16385–16551`, enquanto `29644`/`29599` estão na faixa criada durante a execução dos testes — logo são tabelas do schema de tenant, não tabelas públicas compartilhadas. O que ainda falta para prova direta é a identidade nominal das duas relações, obtenível capturando `pg_locks`/`pg_stat_activity` no momento da falha ou consultando `pg_class` pelos OIDs dentro do próprio CI. Candidatos a avaliar em tarefa própria, agora informados pelo mecanismo real: **(a)** serializar o `TRUNCATE` com `beforeEach`/`afterEach` que aguarde as requisições em voo terminarem, fechando a janela de concorrência; **(b)** dar a `tickets.integration.test.ts` um schema de tenant próprio em vez do compartilhado, eliminando a disputa na origem; **(c)** trocar o `TRUNCATE` por transação com locks explícitos em ordem determinística, compatível com a ordem que as queries do servidor usam.
- ~~4 falhas em `omnichannel.webhooks.integration.test.ts` (e-mail inbound, 401) atribuídas a flakiness~~ — **✅ Resolvido pelo commit `90d8f01`; não era flakiness.** A falha era determinística e reprodutível: os testes autenticavam com `Authorization: Bearer <secret>`, mas a rota valida assinatura Svix (`svix-id`/`svix-timestamp`/`svix-signature` em `email.webhook.ts:185-205`, com `401` em `:510-512`), e `RESEND_WEBHOOK_SECRET` no `.env.test` não tinha formato Svix válido — logo `new Webhook(secret)` sequer construía. O commit introduziu um helper que assina os fixtures com `svix` e corrigiu o segredo para `whsec_...`. Contagem de `it()` no arquivo inalterada (14 antes e depois, sem `.skip`/`.todo`/`.only`), ou seja, os testes foram corrigidos e não desativados. Baseline atual da suíte: **355 testes, 0 falhas**. A contagem saiu de 332 para 335 com `tenant.middleware.integration.test.ts` (4 → 7 casos, commit `afdaf24`), depois 340 no passo 4 (+5 em `omnichannel.webhooks.integration.test.ts`), 350 no passo 5 (+10 com `metrics.integration.test.ts`) e 355 no passo 6 (+5 com `close-config.integration.test.ts`). **Ressalva de escopo (2026-08-02):** a resolução de `90d8f01` cobriu apenas o caminho local — o `.env.test`, carregado pelo script `pnpm test` (`node --env-file=.env.test`). O `.github/workflows/ci.yml` mantinha `RESEND_WEBHOOK_SECRET` no formato inválido antigo e nunca foi sincronizado, então os mesmos testes seguiram falhando no CI, agora com `Base64Coder: incorrect characters for decoding` lançado por `new Webhook(secret)` antes de qualquer requisição. Dois fatos sustentam a separação dos caminhos: o CI não carrega o `.env.test` (chama `pnpm exec vitest` direto, não `pnpm test`), e o bloco `env:` do job tem precedência sobre `--env-file` mesmo se carregasse. O valor do `ci.yml` foi igualado ao do `.env.test` nesta data; a baseline de 355/0 registrada acima refere-se à execução local.
...
- ~~**[BAIXO] `.env.test` aponta para porta Postgres errada**~~ — **✅ Resolvido**: arquivo de teste versionado já aponta para `localhost:5433`; nesta sessão também foi atualizado para usar `RESEND_WEBHOOK_SECRET` em formato Svix (`whsec_...`), permitindo validar os webhooks de e-mail com assinatura real. **Ressalva (2026-08-02):** essa atualização alcançou somente o `.env.test`; o mesmo segredo no `ci.yml` permaneceu em formato inválido até ser sincronizado nesta data — ver a ressalva de escopo no item de `omnichannel.webhooks.integration.test.ts` acima.
...
- ~~**[MÉDIO] `lgpd_requests` com duas definições lazy divergentes (instância confirmada de `0A000`)**~~ — **✅ Fechado o vetor; resíduo de higiene abaixo.** A tabela não existe em `createTenantTables`: nascia lazily por **dois** caminhos independentes com `CREATE TABLE IF NOT EXISTS`, e quem chegasse primeiro no schema definia o shape — `crm.infrastructure.ts` (`request_type VARCHAR(40)`, `status VARCHAR(20) DEFAULT 'processed'`, FKs `ON DELETE SET NULL`) contra `portal.service.ts` (`VARCHAR(30)`, `VARCHAR(30) DEFAULT 'pending'`, FKs sem `ON DELETE`). Mesmos nomes, mesma ordem, mesma contagem: divergia só o `atttypmod`, e isso basta — `equalTupleDescs` (plancache.c) o compara. Os dois caminhos servem semânticas diferentes e ambos inserem: CRM/admin grava trilha de ação **já executada** (nasce `processed`, fora do SLA); portal grava **pedido do titular** (nasce `pending`, com `sla_deadline`). **Correções aplicadas:** (a) shape unificado em `lib/lgpd/schema.ts` (`ensureLgpdRequestsTable`), com `VARCHAR(30)`/`VARCHAR(20) DEFAULT 'pending'` e `ON DELETE SET NULL` — larguras dimensionadas pelos maiores valores reais (`'external_anonymization'` 22 chars, `'processed'` 9 chars); (b) `createLgpdRequestRecord` passou a **sempre qualificar** a tabela por schema (resolvendo por `current_schema()` quando o chamador não informa), e o `RETURNING *` virou lista explícita das 15 colunas. **O que cada camada fecha, medido pela sonda:** a qualificação é o que fecha o vetor — dá a cada schema seu próprio statement no cache do driver, imune a shape divergente; a lista explícita cobre apenas contagem/ordem. **Colunas explícitas sozinhas NÃO blindam typmod** (caso b2 da sonda ainda dispara `0A000`) — não confundir as duas. Oráculo permanente em `apps/api/src/lib/lgpd/__probes__/0A000-lgpd-requests.probe.test.ts` (fora da suíte, `ZIRADESK_PROBE_0A000=1`), com 4 casos: vetor original ainda detectável, defs unificadas, medição do eixo de colunas, e o gate do statement de produção.
  **DÍVIDA RESIDUAL — reconciliação física de `atttypmod`, aplicável só a tenant real legado.** O `0A000` está fechado pela qualificação **para schemas novos e antigos**, mas o shape físico dos tenants já provisionados pelo caminho divergente **continua divergente** até um `ALTER TABLE ... ALTER COLUMN request_type TYPE VARCHAR(30)` / `status TYPE VARCHAR(20)`. É higiene, não correção: sem ela nenhum erro volta, mas o banco segue com duas larguras em produção e qualquer código futuro que compartilhe texto de statement entre tenants reabre o vetor. Se houver tenant real legado, rodar junto de uma revisão do `migrate:lgpd-shape` (`apps/api/src/scripts/migrate-lgpd-and-shape-columns.ts`) em janela controlada — `ALTER COLUMN TYPE` reescreve a tabela e toma `ACCESS EXCLUSIVE`. Sem schemas legados a migrar, o script vira candidato a reavaliação futura, não remoção imediata. Irmão do item "DDL lazy restante ainda pode bloquear requests" acima.
  ~~**Frente B (`conversations`) — 7 `RETURNING *` não-qualificados**~~ — **✅ Fechado por qualificação via `conversationsRef`.** O vetor era diferente de `lgpd_requests`: `conversation_status` é `CREATE TYPE` por schema, então `status` carrega `atttypid`/OID de enum diferente em cada tenant mesmo com os mesmos labels (`open`, `waiting`, `closed`). A correção em `lib/conversations/schema.ts` resolve `current_schema()` quando necessário e monta sempre um literal como `"tenant_demo".conversations`, produzindo texto de statement distinto por schema nos 7 `RETURNING *` de `conversations.service.ts`/`active-outbound.routes.ts`. A sonda `apps/api/src/lib/conversations/__probes__/0A000-conversations.probe.test.ts` mantém o oráculo: (a) nome cru + `RETURNING *` dispara `0A000`; (b) nome qualificado fecha; (c) colunas explícitas incluindo `status`, mas sem qualificação, **ainda disparam** — logo colunas explícitas não fecham o vetor de enum OID; (d) o helper real fecha e confirma `current_schema()` dentro de transação com `SET LOCAL`.
- ~~**[MÉDIO] Restore dos tenants de teste da VPS Contabo anterior como pendência operacional**~~ — **✅ Decisão fechada em 2026-08-01**: `tenant_demo`, `tenant_multsoft_desenvolvimento_de_sistemas` e `tenant_sepol` eram ambientes de teste e **não serão restaurados** a partir do backup no Cloudflare R2. A migração da linha de infraestrutura é independente desta decisão de dados: em qualquer destino novo, tenants devem ser provisionados do zero por `createTenantTables`, com schema limpo. Isso reduz lateralmente a superfície de recorrência do `0A000`, porque esses schemas novos nascem pelo código já corrigido e não carregam shape legado, mas não substitui as correções estruturais de qualificação/provisionamento descritas acima. O backup no R2 (`pg_dump`/rclone em rotina diária) segue existindo como histórico e precaução, sem plano de restore ativo para esses três tenants de teste. Para `agent_bot_skills`, deixa de existir o bloqueio artificial de "backup verificado": sem restore, não há verificação pendente nesses schemas; a permanência ou remoção da tabela deve ser decidida em tarefa própria contra o schema canônico atual, sem assumir dados de teste a preservar.
  **DÍVIDA ORTOGONAL:** `ConversationRow` continua sendo contrato podre e não verificado: mistura campos físicos e campos de JOIN, omite colunas físicas de `conversations` e depende de casts de `$queryRawUnsafe`. Não foi alterado nesta frente para preservar o payload atual do front; tratar em passo dedicado de tipagem/serialização. `users.lgpd.service.ts:121,419` também usam `RETURNING *` sobre `users`, tabela com colunas LGPD adicionadas lazily; mesmo padrão, ainda não auditado. `users.infrastructure.ts:30-53` mantém cópia própria dos `ALTER` de `lgpd_requests` (subconjunto da canônica, sem `CREATE TABLE`) — não reintroduz divergência, mas é candidato a convergir junto.
- **[BAIXO] Códigos HTTP divergentes para o mesmo predicado de tenant bloqueado** — **rebaixado de [MÉDIO] em 2026-08-03, na Etapa 1 do discriminador `TENANT_SUSPENDED`: não é divergência de comportamento ativa.** O `402` de `middleware/tenant.ts:68` é **inalcançável** — `tenantMiddleware` não está registrado em nenhuma rota (ver item próprio abaixo), então em requests reais só o `403` de `tenantSchemaFromJwt.ts` dispara para tenant suspenso/cancelado. O que segue é código morto documentando um caminho que nunca executa. Decisão pendente em tarefa própria: religar o middleware (se houver motivo) ou removê-lo. O levantamento original, preservado por descrever os pontos que **de fato** executam: o predicado `status !== 'active' && status !== 'trial'` é avaliado em pelo menos 6 pontos e cada um responde diferente: **402** em `middleware/tenant.ts:68`, **403** em `middleware/tenantSchemaFromJwt.ts:196` e no login (`auth.routes.ts:65`), **401** no refresh (`auth.service.ts:215` lança `tokenExpired`, mapeado em `auth.routes.ts:280`), **404** no branding do portal (`portal.service.ts:1187`, via `PortalNotFoundError`) e **403** no resto do portal (`portal.service.ts:108`, via `PortalForbiddenError`). O `auth.routes.ts:170` (forgot-password) é caso à parte e correto: devolve `200 {success:true}` de propósito, para não permitir enumeração de tenant. **Padronizar não é trivial, porque os dois candidatos já têm outro dono:** `403` é o código genérico de "sem permissão" no repo (`tenantSchemaFromJwt.ts:158,166` para acesso cross-tenant, `:174,200` para schema inválido, erros de role e LGPD em `users.routes.ts`, claim de ticket em `tickets`), e `402` já significa **limite de plano estourado** (`PlanLimitError` em `users.routes.ts:118`, `contacts.routes.ts:403`, `tenants.routes.ts:193`). Reusar qualquer um dos dois funde "conta suspensa" com um significado existente. Consequência prática hoje: como o refresh devolve `401`, uma suspensão no meio da sessão é indistinguível de sessão expirada — o usuário é mandado ao login, onde recebe `403` com mensagem diferente. Decidir entre código dedicado (ex.: `423 Locked`) ou payload discriminado (`error.code: 'TENANT_SUSPENDED'`) antes de mexer.
- **[MÉDIO] Sem tratamento de conta suspensa no frontend** — irmão do item acima, medido em `apps/web`: **nenhuma ocorrência de tratamento de `402`**, e o interceptor global (`services/api.ts:832`) só reage a `401`, tentando refresh uma vez. Os `403` tratados (`TicketDetail.tsx:457`, `Tickets.tsx:571`, `TVDashboard.tsx:320,333`) são de permissão/claim, nenhum de tenant. Não existe tela ou redirect de "conta suspensa": o usuário de um tenant suspenso vê toast genérico de erro, ou cai no fluxo de sessão expirada pelo `401` do refresh. Fechar junto da decisão de padronização, já que a UI depende de conseguir distinguir o caso.
- **[MÉDIO] Dois mecanismos paralelos para ativar/suspender tenant** — o dropdown da listagem (`Tenants.tsx:524-547`) faz `PATCH /super-admin/tenants/:id { status }`, enquanto o detalhe (`TenantDetail.tsx:118-120`) chama os endpoints dedicados `POST /:id/suspend` e `POST /:id/activate`. Os dois caminhos existem na API (`tenants.routes.ts:109,133,143`) e são funcionalmente equivalentes, já que `updateTenantSchema` aceita `status`. O `PATCH` é o mais capaz (também faz `cancelled`, que não tem endpoint dedicado). Consolidar em tarefa própria: ou os dedicados viram os únicos e ganham `/cancel`, ou são removidos em favor do `PATCH`. Enquanto houver dois, qualquer regra futura de transição de status (auditoria, notificação, guarda) precisa ser escrita duas vezes ou nasce aplicada pela metade.
- **[MÉDIO] Dois endpoints de métricas concorrentes no Super Admin** — `GET /super-admin/metrics/overview` (`metrics.service.ts:getOverview`) alimenta o `Dashboard.tsx` com contagem por status, total de planos, novos em 7/30 dias e tenants por plano; `GET /super-admin/tenants/stats` (`tenants.service.ts:1379`) alimenta os KPIs de `Tenants.tsx` com total, ativos, trials expirando em 7 dias e **MRR estimado** (soma de `plan.priceMonth` dos ativos). Há sobreposição (total e ativos são calculados duas vezes, por caminhos diferentes) e exclusividade em ambos os lados: MRR e "trials expirando" só existem no segundo; `cancelledTenants` é calculado no primeiro e sequer é exibido. Efeito: a métrica de negócio mais relevante não está na tela chamada Dashboard. Unificar num só serviço de métricas do Super Admin, ou definir explicitamente qual endpoint serve qual tela e por quê.
- **[BAIXO] `DELETE /super-admin/plans/:id` não tem consumidor** — a rota existe (`plans.routes.ts:61`) e faz soft delete (`plans.service.ts:57`, grava `isActive: false`), mas nenhuma tela a chama: `Plans.tsx` expõe apenas o toggle, que faz `PATCH { isActive: !plan.isActive }` — funcionalmente idêntico ao `DELETE`, e reversível. Varredura em `apps/web/src/pages/super-admin/` e `components/super-admin/` não encontrou nenhuma UI de exclusão. Ou o endpoint é morto e sai, ou a UI está incompleta e falta a ação — decidir em tarefa própria.
- **[BAIXO] Acoplamento implícito entre a exclusão de `/auth/` no interceptor global e o tratamento manual em `Login.tsx`** — o branch de `TENANT_SUSPENDED` em `services/api.ts` exclui rotas sob `/auth/` (`!original.url?.includes('/auth/')`), porque elas têm caminho próprio: `/auth/refresh` é coberto pelo `catch` de `refreshAccessToken` e `/auth/login` é lido diretamente por `Login.tsx`, que inspeciona `error.code` para escolher entre "credenciais inválidas" e "conta suspensa". Hoje funciona porque **só esses dois** emitem o código dentro do prefixo excluído, cada um com tratamento próprio. Se uma rota nova sob `/auth/` passar a emitir `TENANT_SUSPENDED` — ou outro código que precise do mesmo tratamento —, ela **não é pega por nenhum dos dois caminhos**: o interceptor a ignora por prefixo e não há componente inspecionando o erro. A exclusão de prefixo e o tratamento manual são duas metades de um contrato que não está expresso em lugar nenhum do código. Não bloqueante hoje; revisar se o padrão se repetir, ou trocar a exclusão por lista explícita das rotas que já têm tratamento próprio, para que uma rota nova caia no interceptor por padrão em vez de silenciosamente não ser tratada. Introduzido na Etapa 2 do discriminador `TENANT_SUSPENDED` (2026-08-03).
- **[BAIXO] `middleware/tenant.ts` (`tenantMiddleware`) não é usado por nenhuma rota — candidato a código morto** — varredura em `apps/api/src` não encontrou **nenhuma** referência a `tenantMiddleware` fora da própria definição: não é registrado como hook global em `server.ts` nem aparece em `preHandler` de rota alguma. Todo o enforcement de tenant em requests autenticadas passa por `tenantSchemaFromJwt.ts`. Consequência: as validações do arquivo (tenant não identificado → `400`, não encontrado → `404`, suspenso/cancelado → `402`) nunca executam, e o `request.tenant` que ele popula não é preenchido por esse caminho. Descoberto durante a Etapa 1 do discriminador `TENANT_SUSPENDED` (2026-08-03), ao verificar se a mudança de envelope naquele ponto tinha efeito observável — não tem. Decidir em tarefa própria se o middleware deve ser religado (há resolução por slug/header que `tenantSchemaFromJwt` não faz, então pode haver caso de uso legítimo para rotas não-JWT) ou removido. Enquanto ficar assim, é superfície que aparenta cobertura sem entregá-la — e qualquer correção aplicada nele nasce inerte, como aconteceu na Etapa 1.
- **[BAIXO] `omnichannel.webhooks.integration.test.ts` falha só localmente, e a hipótese da porta foi eliminada** — os 3 testes de e-mail inbound no **caminho de sucesso** (`payload Resend válido cria ticket`, `inbound_email_department_id passa por createTicket`, `remetente sem contato vinculado cria contato`) estouram o `testTimeout` de 15s em execução local; o caminho de rejeição (`sem segredo válido não cria ticket`) passa. A hipótese registrada no CHANGELOG 0.10.14/0.10.15 — "rodou contra o Postgres de desenvolvimento na 5432 em vez do banco de teste na 5433" — **foi testada e eliminada em 2026-08-03**: a execução usou um Postgres de teste dedicado de fato na 5433 (container `pgvector/pgvector:pg16` próprio, `prisma migrate deploy` aplicado), exatamente como o `.env.test` espera, e as 3 falharam igual. A/B com as alterações em `git stash` reproduziu as mesmas 3 na baseline, então também não é regressão de código. Em contraste, o CI (Postgres como service container, runner GitHub-hosted, suíte inteira via `pnpm exec vitest run`) fecha verde nesses mesmos testes — `0ccff72`, `c3fc415`, `b8b679e`, `ab278d7` e `425fe8d` concluíram `success`; a única falha recente, `a07b9e0`, é o deadlock `40P01` registrado em item próprio, não este. **Causa real não diagnosticada.** Suspeita não confirmada: o servidor de teste nasce no `globalSetup`, em processo separado dos workers — item próprio deste §16 já registra que `vi.mock`/`vi.stubGlobal` não alcançam esse processo, permitindo que chamadas saiam para rede real —, combinado com alguma diferença de rede/egress entre a máquina local e o runner do CI. Não bloqueante: o CI é o gate de fato e está verde. Investigação fica para quem precisar rodar a suíte localmente com confiança total.
- ~~**[ALTO] Drift entre `schema.prisma` e o banco real: `tenant_voice_config` existe sem model correspondente**~~ — **✅ Resolvido (2026-08-03)**: `model TenantVoiceConfig` adicionado a `schema.prisma` refletindo o shape real obtido por introspecção — que trouxe três atributos ausentes do registro original deste item: `tenant_id` e `twilio_phone_number` são **UNIQUE** (a relação com `Tenant` é 1:1, não 1:N) e existe **FK `ON DELETE CASCADE`** para `tenants(id)`. Escrever o model a partir da descrição textual, sem introspectar, teria produzido três `ALTER TABLE` residuais. Mapeamentos necessários para o diff zerar: `@default(dbgenerated("gen_random_uuid()"))` (o `@default(uuid())` do Prisma é gerado no cliente e **não** cria default no banco), `onUpdate: NoAction` explícito (o default do Prisma para relação obrigatória é `Cascade`), além de `@db.Uuid`, `@db.VarChar(20)` e `@db.Timestamptz(6)`. `migrate diff` confirmado vazio para essa tabela. **Nenhuma migration criada** — é sincronização do model com tabela já existente, sem mudança de banco. Fechado junto, no mesmo diff, um **segundo drift**: `conversations.waiting_expires_at`/`queue_entered_at` eram `timestamptz` no banco mas `timestamp(3)` sem timezone no model, e um `migrate dev` futuro emitiria a conversão — que no PostgreSQL descarta o offset e rotaciona os valores para o TZ da sessão, perda silenciosa num campo de expiração de fila. `@db.Timestamptz(6)` aplicado aos dois, com consistência confirmada nas três camadas: `public`, o DDL de provisionamento (`tenants.service.ts:550-551`) e schemas de tenant materializados (`tenant_demo`, `tenant_codexa`). Com os dois fechados, o `migrate diff --from-migrations` volta **totalmente vazio**, provando `schema.prisma` sincronizado com o banco `public`.
- **[BAIXO] `conversations.closed_at`/`csat_expires_at` divergem entre `public` (sombra) e os schemas de tenant** — irmão do item [MÉDIO] das tabelas-sombra acima, e a contrapartida do drift de `waiting_expires_at`/`queue_entered_at` fechado no item resolvido logo acima. As duas colunas são `timestamp` sem timezone, precisão 3, em `public` — a tabela-sombra legada e vazia — e `timestamptz(6)` nos schemas de tenant, onde os dados reais vivem (confirmado em `tenant_demo` e `tenant_codexa`, e no DDL de `tenants.service.ts:547,561`). Consequência: o `model Conversation` do Prisma hoje descreve fielmente **a sombra, não os tenants**, nesses dois campos. **Impacto de runtime nulo**: `prisma.conversation.*` não é usado em lugar nenhum do código — varredura por `.conversation.(findMany|findFirst|findUnique|create|update|upsert|delete|count)` retorna zero, todo acesso a conversas é SQL raw —, então o model existe só para tipagem e para o `migrate diff`. O conflito é **irresolvível pelos dois lados ao mesmo tempo**: marcar `@db.Timestamptz(6)` nesses dois campos alinharia o model com os tenants, mas reabriria o diff pedindo a conversão destrutiva `timestamptz → timestamp` na sombra do `public`. Deixar como está mantém o diff vazio e o `public` protegido, ao custo de o model mentir sobre os tenants em dois campos que ninguém lê via Prisma. A decisão de fundo — se `schema.prisma` deve descrever `public` ou o shape de tenant — é a mesma pergunta do item [MÉDIO] das tabelas-sombra: resolver junto com o destino daquelas tabelas, não isoladamente.
- **[BAIXO] Tenant cancelado nunca tem o schema coletado** — `deleteTenant` (`tenants.service.ts:1360`) grava `status: 'cancelled'` com o comentário "preserva dados por 30 dias; schema é mantido", mas **não existe job que colete o schema depois desse prazo**: na prática o schema do tenant permanece indefinidamente, com todos os dados. O bloqueio de acesso funciona (`cancelled` cai no mesmo predicado de `suspended` em todos os pontos de enforcement), então não é vazamento — é retenção não intencional, com implicação de LGPD (dados pessoais preservados sem prazo real) e de custo de armazenamento. Documentado aqui como comportamento atual; limpeza automática — e a decisão de qual prazo de fato aplicar — fica para tarefa própria.

### Ideias registradas — aguardando demanda real (não implementadas)

> Ideias de produto que passaram pelo filtro "demanda real, não especulação" e foram **adiadas até haver sinal de cliente ou decisão estratégica**. São escopo futuro — não dívida técnica no sentido estrito, mas agrupadas aqui no §16 junto dos demais adiamentos (o painel colapsável, o limite de 100 do select), já que todos compartilham o mesmo filtro de "aguardar demanda". Registrar evita perder a ideia e permite escopar *com* a demanda na mão quando ela chegar.

- **Produtos/Serviços por organização** — aba na organização listando o que aquele cliente tem/comprou. Adiada: é ideia de melhoria, sem demanda de cliente (mesmo filtro que adiou o painel colapsável e reverteu os macros). Custo escondido a considerar quando a demanda chegar: (a) exige um **catálogo de produtos** primeiro (não há como associar produto a cliente sem os produtos existirem) — são 2 features, não 1; (b) o **modelo de associação** (quantidade? valor? data? status?) depende de *como* o cliente usaria, que só se sabe com a demanda real; (c) é a **ponta de um módulo** de vendas/pós-venda — uma vez aberto "produtos", vêm pedidos de relatório, histórico, e ligação com tickets ("este ticket é sobre o produto X"). Decisão de posicionamento embutida: o ZiraDesk é central de atendimento ou também CRM de vendas? Escopar só quando um cliente pedir OU quando houver decisão estratégica deliberada de ir para CRM comercial.
