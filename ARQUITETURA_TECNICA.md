...
- **[ALTO] Drift entre `schema.prisma` e o banco real: `tenant_voice_config` existe sem model correspondente** — a tabela foi criada pela migration `20260613120000_add_tenant_voice_config`, existe em produção e é consumida por código real (`admin/voice-config/voice-config.service.ts`, `super-admin/tenants/tenants.service.ts`), mas **não tem model em `schema.prisma`**. Consequência: qualquer `prisma migrate dev` futuro, para qualquer mudança de schema não relacionada, vai incluir `DROP TABLE tenant_voice_config` no diff gerado — silenciosamente, a menos que a migration seja lida linha a linha antes de commitar. Descoberto em 2026-08-03 ao gerar a migration de `public.leads` via `prisma migrate diff --from-migrations` (o `migrate dev` recusa rodar em terminal não-interativo); o diff veio com o `DROP TABLE`, o `DROP CONSTRAINT` da FK e um `ALTER COLUMN ... SET DATA TYPE TIMESTAMP(3)` em `conversations.waiting_expires_at`/`queue_entered_at`. Os drops foram excluídos manualmente daquela migration, que ficou só com o `CREATE TABLE "leads"`, mas **a causa raiz — ausência do model — não foi corrigida**. Correção: adicionar `model TenantVoiceConfig` a `schema.prisma` refletindo o shape real da tabela, como tarefa própria, antes que alguém gere uma migration não relacionada e não perceba o drop no diff. O shape em `public` hoje é `id uuid PK default gen_random_uuid()`, `tenant_id text NOT NULL`, `twilio_phone_number varchar(20) NOT NULL`, `default_bot_menu_id uuid`, `ivr_enabled boolean NOT NULL default true`, `ring_timeout_seconds integer NOT NULL default 20`, `created_at`/`updated_at timestamptz NOT NULL default now()`, mais o índice `idx_tenant_voice_config_phone` em `twilio_phone_number` — confirmar por introspecção (`prisma db pull` num schema isolado, ou `\d tenant_voice_config`) antes de escrever o model, e atentar que `timestamptz` e `uuid` exigem `@db.Timestamptz`/`@db.Uuid` para não gerarem novo drift no próximo diff.
